...
unsigned char* analyse_datagram(){
    /* 调用datalink layer 接受frame */
    receive_frame();
    ...
    while (fread(&Total_length,sizeof(unsigned int),1,file)){     
        ...
        /* 校验checksum */
        unsigned char Header[IHL * 4];
        memcpy(&Header[0],&datagram[0],20+Option_len * 4);
        unsigned short checksum_result = Checksum(Header,IHL * 4);
        if(0 != checksum_result){  // checksum 校验失败
            printf("[Error] checksum error!\n");
            printf("[Error] datagram's checksum : ");
    ...
    /* 组装一个不完整的包含pesudo header的udp，前4位放ip的source ip address
     * 5-8位放des ip address, 然后是空着的段
     * 最后是udp datagram
     */
    unsigned char temp_data_ip_udp[12+payload_total_len];  
    memcpy(&temp_data_ip_udp[0], Source_address, 4);
    memcpy(&temp_data_ip_udp[4], Destination_address, 4);
    memcpy(&temp_data_ip_udp[12], payload, payload_total_len);
